/* This program plays draw poker. Users can play as often as they want, betting between 1 and 5. They are dealt 5 cards and then get to 
choose which cards to keep, and which cards to replace. The new hand is then reviewed and the user's payout is set based on the value of 
the hand. The user's new bankroll is displayed as they are given the option to continue. */
...
void getFinalHand(int carkRank[], int cardSuit[], int finalRank[], int finalSuit[], int ranksinHand[], int suitsinHand[]);
...
int main(){
    ...
    // This function is called outside the do...while loop because the greeting only needs to be displayed once, while everything else in
    // main will run multiple times, depending on how many times the user wants to play.

    printGreeting();

    // Loop runs each time the user play a hand of draw poker
    ...
    do {
        ...
        // These two arrays are used to figure out the value of the player's hand. However, they must be zeroed out in case the user
        // plays multiple hands.
...
// This function looks through each of the five cards in the first hand and asks the user if they want to keep the card. If they say no, 
...
void getFinalHand(int cardRank[], int cardSuit[], int finalRank[], int finalSuit[], int ranksinHand[], int suitsinHand[]){
    ...
    for (i=0; i < 5; i++){
        ...
        if (toupper(ans) == 'Y'){
            ...
            // Next, check the new card against any newly drawn cards to avoid duplication
            for (j = 0; j < i; j++){
